...
%the ratio of data written in most snapshots is lower than 10$\%$, which refers to the amount of data updated in the snapshot. 
%The reason is that the system constantly takes snapshots and the database workload are light-write transactions.
%We also count whether the snapshots are accessed and mark them as red dots.
%The red dots are continuous into short line segments, which means that the snapshots accessed are almost continuous.
